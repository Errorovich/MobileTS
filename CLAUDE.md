...
Many TSLib files are generated by T4 templates and committed alongside their output (`*.gen.cs`, `Generated/*.cs`). The pairing is the `.tt` → `LastGenOutput` `.cs` in [TSLib.csproj](TSLib/TSLib.csproj). **Never hand-edit a generated `.cs`** (e.g. `Messages.cs`, `Book.cs`, `TsFullClient.gen.cs`, `TsCommand.gen.cs`) — edit the `.tt`/`.ttinclude` source and regenerate: Visual Studio's *Transform All Templates* or VS's `Common7/IDE/TextTransform.exe` per file (the `dotnet` CLI does not run these; `dotnet-t4` works but see the newline caveat). Requirements & caveats are in the TSLib repo README: Nett 0.13.0 must sit in the NuGet cache, and the modern T4 engine (VS2022+/dotnet-t4) *swallows the newline after an inline `<# #>` statement block ending a text line* — the templates are formatted with statement blocks on their own lines for this reason; don't reintroduce trailing inline blocks or generated code will silently merge lines (up to code being eaten by a `//` comment).
