...
- TSLib-side layout: connection state machine + send core in [TsFullClient.cs](TSLib/Full/TsFullClient.cs); per-connection state in `ConnectionContext.cs`; crypto handshake in `FullClientHandshake.cs`; voice framing in `Full/Transport/VoicePacket.cs` (packet layer lives in `Full/Transport/`, namespace `TSLib.Full.Transport`; per-connection AES-EAX cipher + TS3INIT1 in `Full/Transport/PacketCipher.cs`); command wrappers in `TsFullClient.Commands.cs`; `TsClientStatus` is a namespace-level enum in `TSLib.Full`. Static identity/crypto utilities (`TsCrypt`, `IdentityData`) are in `TSLib.Crypto`; `ConnectionDataFull`, ID types and enums the app touches are in `TSLib.Shared`.
