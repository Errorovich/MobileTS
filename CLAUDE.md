...
- **TSLib** — `net9.0` class library, a fork of the TeamSpeak 3/5 client library from [Splamy/TS3AudioBot](https://github.com/Splamy/TS3AudioBot), maintained as its own repo ([Errorovich/TSLib](https://github.com/Errorovich/TSLib)) and mounted here as a **git submodule** at `TSLib/`. It in turn has a nested submodule `TSLib/Declarations` ([ReSpeak/tsdeclarations](https://github.com/ReSpeak/tsdeclarations) — protocol declarations consumed by the T4 templates). Uses the net9.0 default C# language version (the historical `LangVersion=8.0` pin from the upstream netstandard2.0 era was removed 2026-07-07). Treat as an upstream dependency: prefer changing MobileTS over editing TSLib; TSLib changes are committed/pushed in its own repo, then the submodule pointer is bumped here. **Layout (restructured 2026-07-08): folder = namespace** — `Shared/` (`TSLib.Shared`: ID types, enums, generated error/permission/version tables, `ConnectionData`, `TsString`/`TsConst`), `ClientBase/` (`TSLib.ClientBase`: `TsBaseFunctions`, `MessageProcessor`, `WaitBlock`), `Crypto/` (`TSLib.Crypto`: `TsCrypt`, `IdentityData`, `License`), `Commands/` (outgoing serialization), `Messages/` (incoming deserialization), `Full/` + `Full/Transport/` (packet layer) + `Full/Book/`, `Query/`, `Audio/` + `Audio/Pipes/` (`TSLib.Audio.Pipes`); module map in the TSLib README. Root-namespace `TSLib` is now empty — types formerly visible everywhere via enclosing-namespace lookup need explicit `using TSLib.Shared;` etc.
...
Many TSLib files are generated by T4 templates and committed alongside their output. Generated files carry the `.gen.cs` suffix and live next to their `.tt` in the semantically matching module (`Full/Book/Book.gen.cs`, `Messages/Messages.gen.cs`, `Shared/TsErrorCode.gen.cs`, `ClientBase/TsBaseFunctions.gen.cs`, …) — a `.tt` and its `.gen.cs` **must stay in one folder** (`LastGenOutput`/`DependentUpon` are bare filenames), and the emitted namespace plus the relative `include`/`Host.ResolvePath` paths are baked into the template, so moving a `.tt` means fixing its insides too. Declaration parsers (`.ttinclude`) follow the same layout (`Messages/MessageParser.ttinclude`, `Full/Book/BookParser.ttinclude`, `Shared/ErrorParser.ttinclude`; shared `Util`/`NotificationUtil` at the root). The pairing is the `.tt` → `LastGenOutput` `.cs` in [TSLib.csproj](TSLib/TSLib.csproj). **Never hand-edit a generated `.cs`** — edit the `.tt`/`.ttinclude` source and regenerate: Visual Studio's *Transform All Templates* or VS's `Common7/IDE/TextTransform.exe` per file (the `dotnet` CLI does not run these; `dotnet-t4` works but see the newline caveat). Requirements & caveats are in the TSLib repo README: Nett 0.13.0 must sit in the NuGet cache; the modern T4 engine (VS2022+/dotnet-t4) *swallows the newline after an inline `<# #>` statement block ending a text line* and *emits the leading whitespace of a standalone control line as output text* — hence statement blocks live on their own lines at column 0 (don't reintroduce trailing inline blocks: generated code will silently merge lines, up to code being eaten by a `//` comment); templates must be CRLF (enforced via `.gitattributes` — the newline-swallowing doesn't recognize bare LF).

### TSLib: known gaps vs protocol declarations (audited 2026-07-06, declarations @ fb4e50d)

The codegen covers all *incoming* protocol surface automatically (message fields, Book structs/updates, versions, errors, permissions). Gaps are in hand-written code, mostly missing c2s command wrappers ([BookToMessages.toml](TSLib/Declarations/BookToMessages.toml) is **not** a codegen input for TSLib — those senders are written by hand; neither is `Enums.toml`, [TsEnums.cs](TSLib/Shared/TsEnums.cs) is manual):

- **`clientupdate` self-state wrappers missing**: InputMuted/OutputMuted/InputHardwareEnabled/OutputHardwareEnabled/Away(+message)/IsRecording/PhoneticName. TSLib only has `ChangeName`/`ChangeBadges`/`ChangeDescription` (+avatar, channel commander, talk request). **The app doesn't wait for these**: mute/afk is already implemented in [MobileTS/Client/Client.cs](MobileTS/Client/Client.cs) by sending raw `new TsCommand("clientupdate") { { "client_input_muted", … } }` through `SendVoid` — `TsCommand` is a generic command builder, a dedicated wrapper is convenience, not a capability gate. Receiving others' mute/afk is fully generated (Book `Client.InputMuted/OutputMuted/AwayMessage`; note `ClientUpdated.IsAway` is `bool?` — only present on state *change*, which is why the app tracks away-state itself).
- **Permission commands missing** (9): `channeladdperm`/`channeldelperm`/`channelpermlist`, `channelclientaddperm`/`channelclientdelperm`/`channelclientpermlist`, `clientaddperm`/`clientdelperm`/`clientpermlist` (only `permissionlist` exists).
- **`channelgetdescription` missing** — Book has `OptionalChannelData.Description` + M2B rule to fill it, but no command to request it from the server.
- **`serveredit`, `logview`** — no wrappers.
- **`TsEnums.LicenseType` is stale**: declarations renumbered it (`1=Offline … 10=Commercial`); TsEnums still has the old `1=Athp, 2=Lan, 3=Npl, 4=Unknown`, so `Book.Server.License` labels are wrong. [License.cs](TSLib/Crypto/License.cs) has a second, *correct* `ServerLicenseType` — duplicated enums that drifted apart. Five-minute fix when needed.
...
- TSLib-side layout: connection state machine + send core in [TsFullClient.cs](TSLib/Full/TsFullClient.cs); per-connection state in `ConnectionContext.cs`; crypto handshake in `FullClientHandshake.cs`; voice framing in `Full/Transport/VoicePacket.cs` (packet layer lives in `Full/Transport/`, namespace `TSLib.Full.Transport`); command wrappers in `TsFullClient.Commands.cs`; `TsClientStatus` is a namespace-level enum in `TSLib.Full`. Identity/crypto (`TsCrypt`, `IdentityData`) are in `TSLib.Crypto`; `ConnectionDataFull`, ID types and enums the app touches are in `TSLib.Shared`.
