...
> **Вторая волна 2026-07-07:** Newtonsoft → System.Text.Json + `readonly record struct` для ID-обёрток (правка `Types.gen.tt` + регенерация T4, коммит `50a4777`), file-scoped namespaces во всех 72 рукописных файлах (`2eedaa3`).
> **Третья волна 2026-07-07 (`0d561b9`):** file-scoped namespaces во всех 10 T4-шаблонах + полная регенерация (проверено `git diff -w` — только строки namespace); CS8073 добавлен в pragma M2B; CS8826 исправлены (генератор объявляет `ProcessEachX(X notify)`, реализации в `TsFullClient.cs` приведены). Предупреждений TSLib: 52 → 13. **Прогон на устройстве после второй/третьей волны ещё не делался.** Ниже только оставшиеся пункты.
...
Внедрять оппортунистически, по мере правок соседнего кода, без big-bang-реформата. Выполнено 2026-07-07: `readonly record struct` для ID-обёрток, file-scoped namespaces везде — рукописные файлы, шаблоны и генерённый код. Осталось:
