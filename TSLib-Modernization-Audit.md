...
> **Статус 2026-07-07:** блоки **1.1–1.4** и **2.1–2.4** выполнены (коммиты TSLib `848f10a`, `5e115f4`), пин `LangVersion=8.0` снят (`f70196a`). Осталось: блок 3 (по мере надобности) и прогон подключения на устройстве (handshake задействует хэши/RNG/FakeDecrypt).
...
## 1. Высокий приоритет — тривиально, реальный выигрыш — ✅ выполнено 2026-07-07
...
## 2. Средний приоритет — тривиально, но меньше выигрыш / нужна аккуратность — ✅ выполнено 2026-07-07
...
## 5. LangVersion 8.0: зачем прописан и можно ли поднять — ✅ пин снят 2026-07-07

**Откуда пин.** Унаследован от upstream (Splamy/TS3AudioBot), где TSLib мульти-таргетил netstandard2.0 (родной максимум — C# 7.3). `LangVersion=8.0` там — принудительный минимум ради `Nullable=enable` (nullable reference types появились в C# 8); из той же эпохи пакет-полифилл `Nullable` и `#if NETSTANDARD*`-ветки. После перехода форка на чистый net9.0 пин не служит ничему — история сабмодуля обрезана («Initial commit»), но других причин в коде нет.

**Можно ли поднять безболезненно — да, проверено сборкой (2026-07-07):** `dotnet build -p:LangVersion=latest` (C# 13 на SDK .NET 9) — **0 ошибок**, и набор предупреждений **побайтно совпадает** с чистой пересборкой на C# 8 (те же 58, включая CS8981 про алиас `str` в генерённых файлах — он завязан на версию *компилятора*, не языка, и светится уже сейчас). LangVersion влияет только на компиляцию, ABI/рантайм не меняются. Правка: удалить строку `<LangVersion>8.0</LangVersion>` из `TSLib.csproj` (net9.0 по умолчанию = C# 13).

**Теоретические риски** (компиляция чистая, но для полноты): в C# 10 у лямбд/method group появились natural types — в редких случаях меняется overload resolution (method group теперь конвертится в `Delegate`/`object`); контекстные ключевые слова (`record`, `required`, `scoped`) конфликтов не дали. Прогон подключения на устройстве после бампа закрывает вопрос.

**Что разблокирует.** Важно: **ничего из блоков 1–3 выше не требует поднятия** — там всё runtime-API. Новая версия языка добавляет только синтаксические улучшения:

| Фича | Где применимо | Ценность |
|---|---|---|
| `readonly record struct` (C# 10) | ID-обёртки в `Types.gen.cs` (`ClientId`, `ChannelId`, `Uid`, …) — сейчас шаблон `Types.gen.tt` генерирует ручные `Equals`/`GetHashCode`/операторы | **Самая осязаемая**: сильно ужимает шаблон и генерённый код; правится `.tt` + регенерация (нюансы T4 — см. README TSLib) |
...
**Рекомендация:** пин снять (однострочный коммит в TSLib-репо + бамп сабмодуля) — это ничего не ломает и убирает искусственное ограничение; новый синтаксис внедрять оппортунистически, по мере правок соседнего кода, без big-bang-реформата. Исключение — `record struct` в `Types.gen.tt`: имеет смысл совместить с уходом от Newtonsoft (п. 3.3), раз всё равно трогается шаблон и T4.
